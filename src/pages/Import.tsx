...
export default function Import() {
  ...
  const bookmarkletSrc = `javascript:(function(){var btns=Array.from(document.querySelectorAll('button'));var exp=btns.find(function(b){return /expand all/i.test(b.textContent);});function grab(){var all=document.body.innerText;var ci=all.indexOf('Course content');var t=ci>-1?all.slice(ci,ci+300000):all.slice(0,300000);var u=location.href;var h1=document.querySelector('h1');var title=h1?h1.textContent.trim():document.title;var w=window.open('${origin}/import','_blank');var sent=false;function send(){if(sent)return;sent=true;w.postMessage({type:'UDEMY_IMPORT',text:t,url:u,title:title},'${origin}');}window.addEventListener('message',function(e){if(e.data==='UDEMY_READY')send();});setTimeout(send,3000);}if(exp){exp.click();setTimeout(grab,2500);}else{grab();}})();`;
